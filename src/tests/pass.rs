...
fn file_settings_simple_file() -> Result<()> {
    let dir = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(dir.path().join(".config").join("rpass"))?;
...
fn file_settings_file_in_xdg_config_home() -> Result<()> {
    ...
    std::fs::create_dir_all(dir2.path().join(".random_config").join("rpass"))?;
    let mut file = File::create(
        dir2.path()
            .join(".random_config")
            .join("rpass")
...
fn rename_file() -> Result<()> {
    ...
    let index = store.rename_file("1/test", "2/test", None)?;
...
fn rename_file_absolute_path() -> Result<()> {
    ...
    store.reload_password_list()?;
    let res = store.rename_file("1/test", "/2/test", None);
...
fn rename_file_git_index_clean() -> Result<()> {
    ...
    store.reload_password_list()?;
    store.rename_file("1/test", "2/test", None)?;
...
fn decrypt_secret() -> Result<()> {
    ...
    let res = pe.secret(&store, None).unwrap();
...
fn test_rename_file_different_sub_permissions() -> Result<()> {
    ...
    store.rename_file("dir/file", "file", None)?;
